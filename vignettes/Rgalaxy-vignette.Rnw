...
The function's arguments must include the names of its input and output
...
Functions should be documented with a manual page. \galaxy{}
...
The \Rcode{galaxy} function takes a number of arguments, and this includes
the names of the arguments of the function you are exposing in Galaxy.
Those arguments are defined as instances of the \Rcode{GalaxyParam} class,
which is aware of the rules of how function parameters in Galaxy must
be defined (as documented at
\url{http://wiki.g2.bx.psu.edu/Admin/Tools/Tool%20Config%20Syntax#A.3Cparam.3E_tag_set}).
In Galaxy, unlike in R, you must specify the data type of each function
argument; the link above (and the \Rcode{GalaxyParam} validity method)
specifies the types you can use. 

Galaxy functions create one or more output files, which are represented
in the call to \Rcode{galaxy} by one or more named instances of the
\Rcode{GalaxyOutput} class. This class constructor takes a single
argument which is the file type of the output file. The list of supported
types can be found in your Galaxy distribution, in the file
datatypes\_conf.xml.sample. 

The third and final class passed to the \Rcode{galaxy} function is
the \Rcode{GalaxyConfig} class, which tells \Rcode{galaxy} where 
you have Galaxy installed and where in the Galaxy hierarchy you want
this function to be visible.
...
<<useful-example>>=
probe_lookup <- function(probe_ids, outputfile)
{
...
<<lookup-probe-lookup>>=
system.file("extdata", "probe_lookup.rd", package="RGalaxy")
@

...you can expose the function in Galaxy as follows:
<<expose-probe-function>>=
galaxy(probe_lookup, 
    name="Look Up Gene Names",
    version="0.0.1",
    manpage=system.file("extdata", "probe_lookup.rd", package="RGalaxy"),
    probe_ids=GalaxyParam(type="text", label="Probe IDs (space separated)"),
